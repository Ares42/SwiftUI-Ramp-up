...
struct ContentView: View {
    ...
    @FetchRequest(
        sortDescriptors: [NSSortDescriptor(keyPath: \ManagedLandmark.id, ascending: true)],
        animation: .default)
    private var landmarks: FetchedResults<ManagedLandmark>

    var body: some View {
        List {
            ForEach(landmarks) { landmark in
              Text("landmark at \(landmark.name!)")
            }
